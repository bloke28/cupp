...
#
#	MODIFIIED BY BLOKE28 FOR 2 KIDS
#              ./cupp-2kids.py -i like usual.
#
...
##### added code for 2 kids #####################################################

    profile["kid1"] = input("> Child's name: ").lower()
    profile["kid1n"] = input("> Child's nickname: ").lower()
    kid1b = input("> Child's birthdate (DDMMYYYY): ")
    while len(kid1b) != 0 and len(kid1b) != 8:
        print("\r\n[-] You must enter 8 digits for birthday!")
        kid1b = input("> Child's birthdate (DDMMYYYY): ")
    profile["kid1b"] = str(kid1b)
    print("\r\n")
...
########################################   ADDED FOR KID2 ##################################
    kid1b_yy = profile["kid1b"][-2:]
    kid1b_yyy = profile["kid1b"][-3:]
    kid1b_yyyy = profile["kid1b"][-4:]
    kid1b_xd = profile["kid1b"][1:2]
    kid1b_xm = profile["kid1b"][3:4]
    kid1b_dd = profile["kid1b"][:2]
    kid1b_mm = profile["kid1b"][2:4]
#############################################################################################
...
####################### ADDED FOR KID 2  #############################################
    kid1up = profile["kid1"].title()
    kid1nup = profile["kid1n"].title()
#####################################################################################
...
############################# ADDED FOR KID 2 ##############################################
    ...
    kbd1s = [kid1b_yy, kid1b_yyy, kid1b_yyyy, kid1b_xd, kid1b_xm, kid1b_dd, kid1b_mm]
    kbd1ss = []
...
############################# ADDED FOR KID 2 ##############################################
    for kbd1s1 in kbd1s:
        kbd1ss.append(kbd1s1)
        for kbd1s2 in kbd1s:
            if kbd1s.index(kbd1s1) != kbd1s.index(kbd1s2):
                kbd1ss.append(kbd1s1 + kbd1s2)
                for kbd1s3 in kbd1s:
                    if (
                        kbd1s.index(kbd1s1) != kbd1s.index(kbd1s2)
                        and kbd1s.index(kbd1s2) != kbd1s.index(kbd1s3)
                        and kbd1s.index(kbd1s1) != kbd1s.index(kbd1s3)
                    ):
                        kbd1ss.append(kbd1s1 + kbd1s2 + kbd1s3)
############################################################################################
                # string combinations....
...
################################### ADDED FOR KID 2  #####################################
    ...
    kombinaaka = []
    for kombina1 in kombinaka:
        kombinaaka.append(kombina1)
        for kombina2 in kombinaka:
            if kombinaka.index(kombina1) != kombinaka.index(kombina2) and kombinaka.index(
                kombina1.title()
            ) != kombinaka.index(kombina2.title()):
                kombinaaka.append(kombina1 + kombina2)
...
################################### ADDED FOR KID 2  #####################################
    kombi[22] = list(komb(kombinaaka, kbd1ss))
    kombi[22] += list(komb(kombinaaka, kbd1ss, "_"))
##########################################################################################
...
################################### ADDED FOR KID 2  #####################################
    kombi[7] = list(komb(kombinaaka, years))
    kombi[7] += list(komb(kombinaaka, years, "_"))
##########################################################################################
    ...
    if profile["randnum"] == "y":
        ...
        kombi[16] = list(concats(kombinaak, numfrom, numto))
################################### ADDED FOR KID 2  #####################################
        kombi[23] = list(concats(kombinaaka, numfrom, numto))
        kombi[23] = list(concats(kombinaaka, numfrom, numto))
##########################################################################################
...
######################## ADDED FOR KID 2 ########################################################
    kombi[19] = list(komb(rev_k, kbd1ss))
    kombi[19] += list(komb(rev_k, kbd1ss, "_"))
#################################################################################################
...
######################## ADDED FOR KID 2 ########################################################
        komb007 = list(komb(kombinaaka, profile["spechars"]))
#################################################################################################
    print("[+] Sorting list and removing duplicates...")
...
######################## ADDED FOR KID 2 ########################################################
    komb_unique013 = list(dict.fromkeys(kombinaaka).keys())
#################################################################################################
